...
- `python3 /home/runner/work/greens-acc.com/greens-acc.com/scripts/trigger_deploy_workflow.py '{"workflow":"deploy.yml","ref":"main"}'` — preflight-check `gh` availability/auth + workflow access, then trigger via `gh workflow run`
...
- For the Python integration server, also set:
   - `SUPABASE_FUNCTIONS_BASE_URL` — optional base URL for edge functions (e.g. `https://<project-ref>.supabase.co/functions/v1` or `https://<project-ref>.functions.supabase.co`)
   - If `SUPABASE_FUNCTIONS_BASE_URL` is not set, the server falls back to `SUPABASE_URL/functions/v1`
