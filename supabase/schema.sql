...
create table if not exists public.green_acc_deals (
  ...
  publisher_type text not null default 'Direct Seller' check (publisher_type in ('Direct Seller','Mandate','Broker')),
  status text not null default 'draft' check (status in ('draft','awaiting_payment','pending_review','published','rejected')),
  payment_intent_id text,
  is_verified boolean not null default false,
  buyer_qualifications text,
...
alter table if exists public.green_acc_deals
  add column if not exists title text,
  add column if not exists description text,
  add column if not exists user_id uuid references auth.users(id),
  add column if not exists publisher_company text,
  add column if not exists publisher_type text default 'Direct Seller',
...
create index if not exists idx_green_acc_deals_status on public.green_acc_deals (status);
